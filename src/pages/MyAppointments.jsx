...
import { fetchAppointments, deleteAppointment, handleCancelledBooking } from '../services/AllAPIs';
...
const MyAppointments = () => {
    ...
    const handleDelete = async (id, appointment) => {
        const confirmDelete = window.confirm(`Do you want to delete the appointment?`);
        ...
        if (confirmDelete) {
            try {
                ...
                const bookingDetails = {
                    user,
                    addressLineOne: appointment.addressLineOne,
                    addressLineTwo: appointment.addressLineTwo,
                ...
                console.log(bookingDetails.addressLineOne);
                handleCancelledBooking(bookingDetails);
                await deleteAppointment(id);

                // Re-fetch the appointments after deletion
                fetchAppointment(user.id);
                alert(`Appointment has been deleted.`);
            } catch (err) {
                console.log("Failed to delete the appointment:", err);
            }
        }
    };
                ...
                {appointments.map((appointment) => (
                    <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={appointment.bookingId}>
                        ...
                        <div className='flex-1 text-sm text-zinc-600'>
                            ...
                            <p className='text-xs'>{appointment.addressLineOne}</p>
                            <p className='text-xs'>{appointment.addressLineTwo}</p>
